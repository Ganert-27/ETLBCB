...
requestApiBcb = Função para extrair os dados da API do Banco Central.

alvarCsv = Serve para salvar o DataFrame em um arquivo CSV (texto separado por vírgula, ponto e vírgula, etc.).

salvarSQLite = Serve para salvar o DataFrame em um banco de dados SQLite (um banco leve que usa arquivos .db).

salvarMySQL = Serve para salvar o DataFrame em um banco de dados MySQL.